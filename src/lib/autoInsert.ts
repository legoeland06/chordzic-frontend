...
export interface AutoInsertState {
  /** Dernier accord vu (avant le tick courant) — clé « label|classes ». */
  prevKey: string | null;
  /** Instant (ms) où prevKey est apparu — null si aucun accord armé. */
  firstSeenAt: number | null;
  /** Dernier accord inséré automatiquement — null si aucun. */
  lastKey: string | null;
}

export const initialAutoInsertState = (): AutoInsertState => ({
  prevKey: null,
  firstSeenAt: null,
  lastKey: null,
});
 ...
 */
export function computeAutoInsert(
  state: AutoInsertState,
  now: number,
  delayMs: number,
  key: string | null,
  insertable: boolean,
): { next: AutoInsertState; shouldInsert: boolean } {
