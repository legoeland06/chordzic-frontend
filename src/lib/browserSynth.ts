...
/** Instrument affecté à un canal pour le rendu WAV (SFZ/VST3/SF2). */
export interface RenderInstrument {
  engine: 'sfz' | 'vst3' | 'fluidsynth' | 'sf2';
  path: string;
}

/** Moteur global dérivé de la sélection : "sfz", "vst3" ou "sf2" si au
 * moins un instrument non-FluidSynth est choisi (SFZ prioritaire si
 * mélangé), sinon undefined → chemin FluidSynth (GM) historique. Chaque
 * canal garde son propre moteur dans `instruments` (le backend accepte
 * {engine, path}). */
export function deriveRenderEngine(
  instruments: Record<number, RenderInstrument>,
): 'sfz' | 'vst3' | 'sf2' | undefined {
  ...
  if (entries.some(e => e.engine === 'sf2')) return 'sf2';
  return undefined;
}
...
export interface RenderOptions {
  ...
  renderer?: Renderer;
  /** Moteur de rendu instruments libres : "fluidsynth" (défaut) | "sfz" |
   * "vst3" | "sf2". Actif seulement si un canal a un instrument non-GM. */
  engine?: 'fluidsynth' | 'sfz' | 'vst3' | 'sf2';
  /** Instruments par canal : { canal: { engine, path } } — le chemin est un
   * .sfz, un preset Surge .fxp, une SoundFont .sf2/.sf3 ou un .vst3.
   * Canal absent → FluidSynth (GM). */
  instruments?: Record<number, { engine: 'sfz' | 'vst3' | 'sf2' | 'fluidsynth'; path: string }>;
}
