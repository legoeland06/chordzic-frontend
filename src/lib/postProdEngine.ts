/**
 * PostProdEngine — moteur audio du mode PostProd.
 *
 * Lecture multipiste synchronisée (un AudioBufferSourceNode par clip, lancés
 * au même instant du contexte audio), VU réels par piste (AnalyserNode) et
 * export WAV via OfflineAudioContext (le rendu offline = exactement le graphe
 * de lecture, clips + fades + gains + faders + pan + master).
 */
import { PostProdSession, PostProdTrack } from './postProdTypes';
import { backendUrl } from './chordUtils';

export class PostProdEngine {
  session: PostProdSession | null = null;
  /** Callback de niveaux (VU) : (levels par piste, master). Appelé par la vue. */
  onLevels: ((levels: number[], master: number) => void) | null = null;
  ...
  setMasterVolume(v: number) {
    this.masterVolume = v;
    if (this.masterGain) this.masterGain.gain.value = v * (this.session?.masterGain ?? 1);
  }

  loadSession(session: PostProdSession) {
    this.stop();
  ...
  setSession(session: PostProdSession) {
    this.session = session;
  }
  ...
  private async getCtx(): Promise<AudioContext> {
    ...
    if (this.ctx.state === 'suspended') {
      try { await this.ctx.resume(); } catch { /* silencieux */ }
    }
    return this.ctx;
  }
  ...
  async decodeWav(url: string): Promise<AudioBuffer> {
  ...
  private audibleTracks(): PostProdTrack[] {
   ...
   * cours de lecture).
   */
  private buildGraph(ctx: BaseAudioContext, when: number, startSec: number, isOffline: boolean) {
  ...
  getPosition(): number {
    ...
    if (this._playing && this.ctx) {
      ...
      if (this._loop && dur > 0) p = ((p % dur) + dur) % dur;
  ...
  getLevels(): { levels: number[]; master: number } {
