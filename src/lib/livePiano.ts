...
export function chromaticOf(pitch: number): number {
 ...
 * la plage du piano.
 */
export function pitchToGraphIndex(
  pitch: number,
  pitchMin = LIVE_PIANO_MIN_PITCH,
  pitchMax = LIVE_PIANO_MAX_PITCH,
): number {
  if (pitch < pitchMin || pitch > pitchMax) return -1;
 ...
 */
export function activePitchSet(
  ...
  return new Set(
    activePitches.filter(
      p => Number.isInteger(p) && p >= pitchMin && p <= pitchMax,
    ),
  );
}
