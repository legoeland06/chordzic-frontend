/**
 * 🎛 ExpressionFrame — cadre commun des modules MPE « expression ».
 *
 * Tous les contrôleurs d'expression (Seaboard, ROLI Seaboard RISE 2,
 * LinnStrument, Osmose…) partagent le MÊME cadre : la zone de manipulation
 * (fournie par le module) + les réglages fins (sliders bend/pression/timbre,
 * range ±, LFO, cible du son, instrument GM, retour auto) + l'état temps
 * réel (notes tenues, canal résolu, bend effectif, REC).
 *
 * Le module fournit seulement son `pad` (la zone plein écran) qui reçoit
 * `returnMode` / `onGesture` / `onGestureEnd` — le state partagé et la
 * communication serveur vivent ici (pas de duplication).
 */
import { memo, useCallback, useEffect, useState, ComponentType } from 'react';
...
/** Props reçues par la zone de manipulation d'un module d'expression. */
...
interface ExpressionFrameProps {
  /** Titre de la modal (nom du contrôleur). */
  title: string;
  /** Icône (emoji). */
  icon: string;
  onClose: () => void;
  /** La zone de manipulation du module (plein écran). */
  pad: ComponentType<MpeModuleProps>;
}

function ExpressionFrame({ title, icon, onClose, pad: Pad }: ExpressionFrameProps) {
  ...
  return (
    <div className="fixed inset-1 sm:inset-2 z-50 flex items-stretch bg-black/70 backdrop-blur-sm p-1 sm:p-2">
      <div className="w-full max-w-[1400px] mx-auto bg-[#141a24] border border-gray-700 rounded-2xl shadow-2xl p-2 sm:p-3 flex flex-col gap-2 max-h-full">
        ...
        {/* ── Zone de manipulation du module (plein écran) ── */}
        <Pad returnMode={returnMode} onGesture={handleGesture} onGestureEnd={handleGestureEnd} />

        {/* ── Réglages fins (communs à tous les modules d'expression) ── */}
...
export default memo(ExpressionFrame);
