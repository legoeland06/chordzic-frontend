...
function lineEl(): HTMLElement {
  const el = document.querySelector('div[style*="translateX"]');
  if (!el) throw new Error('ligne de lecture introuvable');
  return el as HTMLElement;
}

function renderLine(scale: number, contentWidth?: number): Root {
  const root = createRoot(document.body);
  act(() => {
    root.render(<PlayheadLine scale={scale} contentWidth={contentWidth} />);
  });
  return root;
}
...
describe('<PlayheadLine />', () => {
  ...
  it("recalcule la position quand l'échelle change SANS notification du store", () => {
    ...
    act(() => {
      root.render(<PlayheadLine scale={4} />);
    });
  ...
  it('recalcule quand la largeur du contenu change (masquage hors champ)', () => {
    ...
    act(() => {
      root.render(<PlayheadLine scale={1} contentWidth={500} />);
    });
