...
export default function ClickControl() {
    ...
    fetch('/click')
      ...
      .then((d) => {
        setCfg({
          ...
          in_render: d.in_render, out_device: d.out_device || null, delay_ms: d.delay_ms,
        });
        setSounds(d.sounds || []);
      })
      .catch(() => {});
  ...
  return (
    <div className="flex items-center gap-1.5 shrink-0 px-1 py-1 rounded-lg border border-gray-800 bg-gray-900/60">
      ...
      {/* Latence (calage du mode séparé) */}
      {separated && (
        <>
          <input
            type="range" min={0} max={100} value={cfg.delay_ms}
            onChange={(e) => apply({ delay_ms: parseInt(e.target.value) })}
            title={`Calage du clic séparé (${cfg.delay_ms} ms) — si le clic est en retard, diminuez ; en avance, augmentez.`}
            className="w-12 accent-amber-500"
          />
