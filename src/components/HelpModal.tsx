...
export default function HelpModal({ show, onClose }: HelpModalProps) {
  ...
  const sections = [
    ...
    { id: 'push3', icon: '🥁', title: 'Push 3 — pads', keys: 'push ableton pads sample échantillon import wav mp3 ogg flac retrigger drum machine loop boucle couleur dégradé palette peinture peindre tempo bpm métronome quantifié synchronisé stop lecture navigateur serveur ffplay déclencher' },
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-4 sm:px-6 py-4">
          ...
          <Section id="mpe" icon="🎛" title="MPE — expression" onSpeak={speakSection} speaking={speakingId === 'mpe'}>
            ...
            <Row k="🥁 Push 3 — pads" v="64 pads échantillonnés (import de samples, retrigger, mode loop par défaut, couleur posable par pad, tempo importé par pad, métronome synchronisé, lecture navigateur OU serveur, stop) — voir la section dédiée plus bas." />
          ...
          <Section id="push3" icon="🥁" title="Push 3 — pads" onSpeak={speakSection} speaking={speakingId === 'push3'}>
            ...
            <Row k="🔁 Mode LOOP (défaut)" v="Par défaut, les samples BOUCLENT en continu jusqu'au ■ Stop : tu construis des boucles rythmiques en empilant les pads. Un nouvel appui sur un pad en boucle le redéclenche depuis le début (retrigger). Désactive le toggle 🔁 pour un déclenchement one-shot." />
            ...
            <Row k="🖥 / 🖧 Chemin de lecture" v="Au choix : 🖥 Navigateur = les samples sont joués par le navigateur (Web Audio, synchronisation exacte avec le métronome) ; 🖧 Serveur = ils sont joués par le serveur (ffplay, sortie audio du PC) — le métronome reste local, les déclenchements sont anticipés pour arriver sur le battement. Choix persistant." />
          ...
          <Section id="pistes" icon="🎚️" title="Pistes & réglages" onSpeak={speakSection} speaking={speakingId === 'pistes'}>
            ...
            <Row k="🎹 Roll" v={<><b>Bouton du mixeur</b> (mode 📱 Navig.) : sur chaque carte de piste, « 🎹 Roll » ouvre/ferme le <b>Piano Roll intégré</b> de cette piste (équivalent au clic sur son nom dans les pistes). Dans la barre d'outils du Piano Roll intégré, le bouton <b>⛶</b> ouvre le même Piano Roll en <b>modal plein écran</b> (100 % de l'écran, hauteur et largeur — le registre s'étend automatiquement pour remplir toute la hauteur) pour travailler à de meilleures échelles — les deux restent <b>parfaitement synchronisés</b> (mêmes notes, modification instantanée des deux côtés). <b>Sélecteur de piste</b> en haut du modal : sauter d'une piste à l'autre <b>sans fermer</b> (même position de lecture / scroll / zoom — seules les notes changent).</>} />
            <Row k="🎹 LivePiano" v={<>
          ...
          <Section id="pianoroll" icon="🎹" title="Piano Roll" onSpeak={speakSection} speaking={speakingId === 'pianoroll'}>
            ...
            <Row k="🔍 Zoom / défilement" v="Ctrl+molette ou G/H = zoom horizontal (bornes : fit-to-width → 400 %) ; molette simple = scroll vertical du registre (1 demi-ton par cran, « de case en case ») ; Shift+molette = défilement horizontal. Le modal s'ouvre calé sur tout le morceau (fit-to-width) et le registre remplit toute la hauteur de l'écran (fit-to-height, marge clavier comprise)." />
            ...
            <Row k="🎹 Clavier en marge" v="Le clavier de piano vertical (à droite) se rétracte/réaffiche via le bouton 🎹 posé sur la marge elle-même (préférence mémorisée). Sur une piste de batterie (canal 9), la marge affiche les NOMS des percussions (Grosse caisse, Caisse claire, Hi-Hat, Ride, Crash…) — ceux du kit SFZ assigné dans 🎛️ s'il y en a un, sinon le kit GM — pour placer les notes au bon endroit sans compter les touches." />
