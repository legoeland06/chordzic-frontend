...
describe('<ChordNowModal />', () => {
  it('ne rend rien hors lecture (pas de modal fantôme)', () => {
    expect(renderToString(<ChordNowModal chords={chords} highlighted={0} playing={false} />)).toBe('');
    expect(renderToString(<ChordNowModal chords={chords} highlighted={-1} playing={true} />)).toBe('');
    expect(renderToString(<ChordNowModal chords={[]} highlighted={0} playing={true} />)).toBe('');
  });

  it('affiche l accord courant en très gros (cercle translucide)', () => {
    const html = renderToString(<ChordNowModal chords={chords} highlighted={0} playing={true} />);
  ...
  it('affiche un tiret pour un silence (_)', () => {
    const html = renderToString(<ChordNowModal chords={chords} highlighted={2} playing={true} />);
    expect(html).toContain('\u2014');
  });

  it('affiche la basse (C/G) telle quelle', () => {
    const html = renderToString(<ChordNowModal chords={chords} highlighted={3} playing={true} />);
