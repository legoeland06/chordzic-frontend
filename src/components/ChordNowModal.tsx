...
interface ChordNowModalProps {
  chords: { time: number; chiffrage: string }[];
  highlighted: number;
  playing: boolean;
}
...
export default function ChordNowModal({ chords, highlighted, playing }: ChordNowModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-40 flex items-center justify-center pointer-events-none">
      {/* Cercle translucide */}
      <div className="w-72 h-72 sm:w-96 sm:h-96 rounded-full bg-gray-950/50 backdrop-blur-sm border border-gray-700/40 shadow-[0_0_80px_rgba(0,0,0,0.6)] flex flex-col items-center justify-center gap-1.5 px-6">
        <div className="text-[10px] uppercase tracking-[0.25em] text-gray-500 select-none">En lecture</div>
        {/* Accord courant — très gros */}
        <div
          className="text-7xl sm:text-8xl font-bold font-mono leading-none select-none"
          style={{ color: getChordColor(highlighted) }}
          title="Accord joué par la séquence"
        >
          {current.chiffrage === '_' ? '\u2014' : current.chiffrage}
        </div>
        {/* Accord suivant (petit, transparent) */}
        {next && (
          <div
            className="text-xl sm:text-2xl font-mono opacity-40 select-none"
